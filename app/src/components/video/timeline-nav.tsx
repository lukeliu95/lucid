      ...
      {items.map((it, i) => {
        ...
        return (
          <li key={i}>
            <button
              type="button"
              aria-label={`${formatTimestamp(it.timestamp_sec)} · ${it.title}`}
              className={cn(
                "grid w-full cursor-pointer grid-cols-[56px_1fr] gap-2 border-l-2 py-2 pl-3 text-left transition-colors duration-fast",
                active ? "border-amber-600 bg-bg-elev" : "border-transparent hover:bg-bg-elev",
              )}
              onClick={() => {
                setActiveIdx(i);
                seek(it.timestamp_sec);
              }}
            >
